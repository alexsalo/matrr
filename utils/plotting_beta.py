...
def create_christa_graphs():	
	...
	for i in range(3):
		...
		for fig in volbout_figs:
			DPI = fig.get_dpi()
			filename = output_path + '%s.Phase%d.png' % ("cohorts_daytime_volbouts_bargraph_split", i)
			fig.savefig(filename, dpi=DPI)
		for fig in bouts_figs:
			DPI = fig.get_dpi()
			filename = output_path + '%s.Phase%d.png' % ("cohorts_daytime_bouts_histogram_split", i)
			fig.savefig(filename, dpi=DPI)
	create_age_graphs()
