...
def _create_cbt(bouts, date, cbt_number=0, overwrite=False, minutes_to_exclude=0):
	"""
	...
	if len(bouts):
		...
		cbt, is_new = CohortBout.objects.get_or_create(dex_date=date, cbt_number=cbt_number, cbt_pellet_elapsed_time_since_last=minutes_to_exclude)
		...
		for index, bout in enumerate(bouts):
			...
			if bout_gap >= 300:
				...
				_create_cbt(bouts[index:], date, cbt_number+1, overwrite=overwrite, minutes_to_exclude=minutes_to_exclude)
...
def create_cohort_bouts(cohort, overwrite=False, minutes_to_exclude=0):
	if not isinstance(cohort, Cohort):
		try:
			cohort = Cohort.objects.get(pk=cohort)
		except Cohort.DoesNotExist:
	...
	for date in all_dates:
		...
		bouts = bouts.filter(ebt_contains_pellet=False, ebt_pellet_elapsed_time_since_last__gte=minutes_to_exclude*60)
		bout_values = bouts.values('ebt_start_time', 'ebt_end_time')
		# And send the times into a recursion loop
		_create_cbt(bout_values, date, cbt_number=0, overwrite=overwrite, minutes_to_exclude=minutes_to_exclude)
