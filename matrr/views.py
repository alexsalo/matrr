...
def cohorts_view_assay(request):
    return redirect(
        reverse('tissue-shop-landing', args=[Cohort.objects.get(coh_cohort_name__iexact="Assay Development").pk, ]))
    ...
    if request.method == 'POST':
        if 'request_form' in request.POST:
            if settings.PRODUCTION:
                ...
                datetime.now().strftime("%Y-%m-%d,%H:%M:%S"), settings.PRODUCTION)
...
@user_owner_test(
    lambda u, req_id: u == Request.objects.get(req_request_id=req_id).user or u.has_perm('matrr.change_shipment'),
    arg_name='req_request_id', redirect_url='/denied/')
...
@user_owner_test(lambda u, req_id: u == Request.objects.get(req_request_id=req_id).user, arg_name='req_request_id',
                 redirect_url='/denied/')
def order_revise(request, req_request_id):
    req = Request.objects.get(req_request_id=req_request_id)
    if not req.can_be_revised():
        raise Http404('This page does not exist.')
    return render_to_response('matrr/order/order_revise.html', {'req_id': req_request_id},
                              context_instance=RequestContext(request))
...
def order_duplicate(request, req_request_id):
    ...
    return render_to_response('matrr/order/order_duplicate.html', {'req_id': req_request_id,
...
@user_owner_test(lambda u, rtt_id: u == TissueRequest.objects.get(rtt_tissue_request_id=rtt_id).req_request.user,
                 arg_name='req_rtt_id', redirect_url='/denied/')
...
def inventory_cohort(request, coh_id):
    ...
    return render_to_response('matrr/inventory/inventory_cohort.html',
                              {"cohort": cohort, "monkeys": monkeys, "matrix": availability_matrix},
                              context_instance=RequestContext(request))
...
def inventory_brain_cohort(request, coh_id):
    ...
    return render_to_response('matrr/inventory/inventory_brain_cohort.html', {"cohort": cohort},
                              context_instance=RequestContext(request))
...
def inventory_brain_monkey(request, mky_id):
    ...
    if show_grid:
        ...
        for tst in TissueType.objects.filter(category__cat_name__icontains='brain').order_by('tst_tissue_name'):
            ...
            for block in blocks:
                brain = list()
                left_hemi = MonkeyBrainBlock.objects.get(monkey=monkey, mbb_hemisphere='L',
                                                         mbb_block_name__contains=block)
                right_hemi = MonkeyBrainBlock.objects.get(monkey=monkey, mbb_hemisphere='R',
                                                          mbb_block_name__contains=block)
    ...
    context = {"plot_gallery": True, "monkey": monkey, 'brain_form': brain_form, 'image': image, 'matrix': matrix,
               'blocks': blocks, 'show_grid': show_grid}
    return render_to_response('matrr/inventory/inventory_brain_monkey.html', context,
                              context_instance=RequestContext(request))
...
def sendfile(request, id):
    ...
    for r, f in files:
        if len(r) > 0:
            ...
            break
    if not file:
        return HttpResponseRedirect(settings.STATIC_URL + 'images/permission_denied.jpg')
...
def raw_data_upload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            destination.close()
            return render_to_response('upload_forms/raw_data_upload.html',
                                      {'form': RawDataUploadForm(), 'success': True},
                                      context_instance=RequestContext(request))
    else:
        form = RawDataUploadForm()
    return render_to_response('upload_forms/raw_data_upload.html', {'form': form},
                              context_instance=RequestContext(request))
