...
def monkey_bec_correlation(monkey):
    if MonkeyBEC.objects.filter(monkey=monkey).count() < 10: # arbitrary call
        ...
        return _monkey_bec_correlation(monkey)
...
def _monkey_bec_correlation(mky):
    ...
    print 'monkey: %s \nbecs retained: %s' % (mky, becs_retained.count())
    print 'etoh on prev day retained: %s' % mtds_prev_retained.count()
    print 'etoh on next day retained: %s \n' % mtds_next_retained.count()
    ...
    fig, axs = pyplot.subplots(1, 3, figsize=(15, 8), facecolor='w', edgecolor='k')
    ...
    def plot_regression_line_and_corr_text(ax, x, y):
        ...
        text = 'Correlation: %s' % numpy.round(x.corr(y), 4)
