...
urlpatterns += patterns('matrr.views',
  ...
  url( r'^monkeys/(?P<monkey_id>\S+)/$',
      monkey_detail_view),
  url( r'^(available|upcoming)/(?P<cohort_id>\d+)/tissues/(?P<tissue_model>regions|peripherals)/$',
      tissue_list),
  url( r'^(available|upcoming)/(?P<cohort_id>\d+)/tissues/(?P<tissue_model>regions|peripherals)/(?P<tissue_id>\d+)/$',
