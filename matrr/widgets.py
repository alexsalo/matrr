...
class CheckboxSelectMultipleSelectAll(CheckboxSelectMultiple):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        output = [
            u'<fieldset"><legend><input type=\'checkbox\' id=\'%s\' onclick=\'toggle_checked(this, "%s")\'> <label for=\'%s\'>Select All</label></legend>' % (
        ...
        for i, (option_value, option_label) in enumerate(chain(self.choices, choices)):
            ...
            output.append(u'<div style="display: inline-block;white-space: nowrap;"><label%s>%s %s</label></div>' % (
...
class MonkeyCheckboxSelectMultipleSelectAll_DefaultShown(CheckboxSelectMultipleSelectAll):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        output = [
            u'<fieldset id="monkey_fieldset" style=""><legend><input type=\'checkbox\' id=\'%s\' onclick=\'toggle_checked(this, "%s")\'> <label for=\'%s\'>Select All Monkeys</label></legend>' % (
            attrs['id'], name, attrs['id'])]
...
class MonkeyCheckboxSelectMultipleSelectAll_DefaultHidden(CheckboxSelectMultipleSelectAll):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        output = [
            u'<fieldset id="monkey_fieldset" style="display:none"><legend><input type=\'checkbox\' id=\'%s\' onclick=\'toggle_checked(this, "%s")\'> <label for=\'%s\'>Select All Monkeys</label></legend>' % (
            attrs['id'], name, attrs['id'])]
...
class CheckboxSelectMultipleLinkByTable(CheckboxSelectMultipleLink):
    ...
    def get_status(self, monkey):
        verification, is_new = self.tissue.tissue_verification_set.get_or_create(monkey=monkey,
                                                                                 tissue_request=self.tis_request)
...
class FixTypeSelection(Input):
    ...
    def render(self, name, value, attrs=None, choices=()):
        if value is None: value = ''
        final_attrs = self.build_attrs(attrs, name=name)
        output = [u'<select id=\'%s_selection\' onchange=\'updateSelection("%s")\'>' % (
        final_attrs.get('id'), final_attrs.get('id'))]
...
class CheckboxSelectMultiple_nolist(forms.CheckboxSelectMultiple):
    ...
    def render(self, *args, **kwargs):
        ...
        return mark_safe(
            output.replace(u'<ul>', u'').replace(u'</ul>', u'').replace(u'<li>', u'').replace(u'</li>', u'<br>'))
...
class CheckboxSelectMultipleSelectAll_columns(CheckboxSelectMultiple_columns):
    def _base_output(self, name, value, attrs=None, choices=()):
        select_all_checkbox = u'<legend><input type=\'checkbox\' id=\'%s\' onclick=\'toggle_checked(this, "%s")\'> <label for=\'%s\'>Select All</label></legend>' % (
        attrs['id'], name, attrs['id'])
...
class CheckboxSelectMultiple_proteinAdvSearch(forms.CheckboxSelectMultiple):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        if value is None: value = []
        has_id = attrs and 'id' in attrs
        final_attrs = self.build_attrs(attrs, name=name, onclick="post_adv_form()")
        output = [
            u'<table style="width=80%"><tr><td><label for="select_all_proteins"><input id="select_all_proteins" type="checkbox" name="proteins" onclick="toggle_named_checkboxes(this)"/> Select All Proteins</label></td>']
        # Normalize to strings
