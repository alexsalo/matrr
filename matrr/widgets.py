...
class MonkeyCheckboxSelectMultipleSelectAll_DefaultHidden(CheckboxSelectMultipleSelectAll):
	...
	def render(self, name, value, attrs=None, choices=()):
		...
		output = [u'<fieldset id="monkey_fieldset" style="display:none"><legend><input type=\'checkbox\' id=\'%s\' onclick=\'toggle_checked(this, "%s")\'> <label for=\'%s\'>Select All Monkeys</label></legend>' % (
		attrs['id'], name, attrs['id'])]
...
class CheckboxSelectMultiple_proteinAdvSearch(forms.CheckboxSelectMultiple):
	...
	def render(self, name, value, attrs=None, choices=()):
		...
		output = [u'<table style="width=80%"><tr><td><label for="select_all_proteins"><input id="select_all_proteins" type="checkbox" name="proteins" onclick="toggle_named_checkboxes(this)"/> Select All Proteins</label></td>']
		# Normalize to strings
